...
export class IntermediateLevelComponent implements OnInit {
  private multiCaster$: AsyncSubject<number> = new AsyncSubject<number>();
  ...
  private asyncSubject() {
     ...
     * Note that: After the multiCaster$ is completed, calling next() (or error()) will not have any effect.
     * Also observe nothing is logged on subscription, only after completion multiCaster$ will emit the last value
     */

    this.multiCaster$.subscribe({
      next: (value) => console.log('Subscriber 1 received value ->', value),
      error: (error) =>
        console.log('Error happened for 1st subscriber: ', error),
    ...
    this.multiCaster$.subscribe({
      ...
      error: (error) =>
        console.log('Error happened for 2nd subscriber: ', error),
    ...
    this.multiCaster$.subscribe({
      ...
      error: (error) =>
        console.log('Error happened for 3rd subscriber: ', error),
